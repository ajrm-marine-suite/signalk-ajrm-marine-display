/**
 * Renders the Weather Database's provider-neutral hourly weather and marine
 * projection. These columns deliberately match the Weather Database webapp.
 */

export const forecastColumns = Object.freeze([
...
function instant(source, timezone) {
	...
	const parsed = Date.parse(
		hasZone || utcProvider ? (hasZone ? source : `${source}Z`) : source,
	);
...
export function forecastRows(projection) {
	...
	return hourly.time.map((time, index) => ({
		...
		windDirection: cardinal(
			hourly.wind_direction_10m?.[index] ?? hourly.winddirection_10m?.[index],
		),
...
export function renderForecastTable(table, projection) {
	...
	const documentObject = table.ownerDocument || globalThis.document;
	table.querySelector("thead").innerHTML = `<tr>${forecastColumns
